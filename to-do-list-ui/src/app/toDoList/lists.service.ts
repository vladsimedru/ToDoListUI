...
export class ListsService {
  ...
  addToDoListTask(toDoListId: number, description: string): Observable<number> {
    return this.http
      .post<any>(this.url + `/${toDoListId}/task`, { description })
      .pipe(
        tap((data) => console.log('All', JSON.stringify(data))),
        catchError(this.handleError)
      );
  ...
  updateToDoListTaskStatus(id: number, status: boolean): Observable<ITask> {
    const body = { status };
    return this.http
      .put<any>(this.url + `/task/${id}?status=${status}`, body)
      .pipe(
        tap((data) => console.log('All', JSON.stringify(data))),
        catchError(this.handleError)
      );
  ...
  deleteToDoListById(id:number):Observable<any>{
